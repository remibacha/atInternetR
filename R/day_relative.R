...
#' @param period_duration Analysis period
#'
...
#'
#' @return day_relative extract the data you need from 'AT Internet' database, according to parameters you entered and then provides a dataset.
#'
#' @import httr
#' @import data.table
#'
...
day_relative <- function(ATuser, ATpassword, space, period_duration, dim_source = NULL, dim_device = NULL, dim_level2 = NULL, segment = NULL, pagination=1){
  ...
  for (i in 1:period_duration) {
    ...
    api_request <- paste0("https://apirest.atinternet-solutions.com/data/v2/json/getData?",
                          ...
                          "&sort={d_time_date}",
                          "&space={s:'", space,"'}",
                          "&period={R:{D:{start:'-",time,"',end:'-",time,"'}}}",
